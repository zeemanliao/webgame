...
coms.create = function(chara,data){
	...
	Team.create(chara,map,pwd,function(err,team){
		...
		Game.in(map_room,{team:{addTeam:self.parseReturnTeam(team)}});
...
coms.join  = function (chara,data) {
	...
	//請先選擇地圖
	if (!chara.tmp.map)
		return Game.show_message(chara,lang.err.E0018);
	var team = chara.tmp.map.teams[team_id];
	//找不到隊伍編號「$」!
	if (!team)
		return Game.show_message(chara,lang.err.E0019,team_id);
	team.join(chara,pwd,function(err){
		if (err)
			return Game.show_message(chara,err);

		if (team.count()>=Game.settings.game.teamMax){
			Game.in('map_'+team.map.id,{team:{removeTeam:team.id}});
		} else {
			Game.in('map_'+team.map.id,{team:{updateTeam:self.parseReturnTeam(team)}});
		}
		//return Game.show_message(chara,'加入'+chara.tmp.team.id);		
	});
}
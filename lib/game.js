...
*/
Game.coms = coms;
console.log('components load Start...............');
/*
...
console.log('components loading End...............');
...
Game.on = function(chara,data){
	...
	if (!data.com)
		return;
	if (typeof Game.coms[data.com] == "object"){
...
Game.emit = function(chara,com,com2,data){
	chara.updateData({com:com,com2:com2,data:data});
}
/*
	載入資料
*/
...
Game.update = function(chara,data){
	...
	var redata = {chara:{update:chara.allData()},
								sys:{
									conf:settings.game,
									ver:ver,
									db:{}
...
Game.join = function (chara,next){
		...
		//載入相關資料到頁面
		/*
		chara.updateData({chara:chara.allData(),
									conf:settings.game,
									maps:db.maps,
									pets:db.pets,
									skills:db.skills,
		...
		this.map.on(chara,'move',chara.data.map);
		*/
...
Game.show_message = function(chara,msg,next){
			if (msg){
				chara.socket.emit('show message',{msg:msg});
			}
			if (next)
				next(null);
		};
...
function put_data(dname,file){
	...
	fs.stat(js_path+'/data/'+file, function(err,stat){
	  ...
	  ver[dname] = d.getTime();
		for (var i in data) {
				if (c[data[i].id]) {
					console.log('(file:%s) id:%s資料重複',file,data[i].id);
				}else{
					c[data[i].id]=data[i];
				}
		}
		console.log("data %s..........Load ok",file);
	});
}
...
module.exports = {
	levelup:function(player, itemID, callback) {
		try {
			...
			if (_mixSourceItem.id == item.id) {
				if (_mixSourceItem.data.num == 0) {
					_mixSourceItem.data.num = 1;
					_mixSourceItem.data.level +=1;

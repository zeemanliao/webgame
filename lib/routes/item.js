...
module.exports = {
	...
	move:function(player, itemID, callback) {
		...
		var _item = player.storage.getTheSameItem(item.data);
		if (tool.isUndefinedOrNull(_item)) {
	...
	equip:function(player, itemID, callback) {
		...
		var _oldStorage = item.data.storage;
		item.data.storage = this.game.settings.game.storageType.equipment;

		if (!tool.isUndefinedOrNull(player.storage.getTheSameItem(item.data))){
			item.data.storage = _oldStorage;
			return callback(null);
		}
		item.data.storage = _oldStorage;
		this.remove(player, item.base.type, function(err){
			...
			var _data = {item:{}};
			if (item.data.num == 1) {
				item.data.storage = self.game.settings.game.storageType.equipment;

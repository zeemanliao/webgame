...
module.exports = {
	...
	levelUp:function(player, itemID, callback) {
		try {
			...
			if (_useGold > player.data.gold)
				return callback(new this.game.GameWarning('E0032', item.nam));

			var stones = this.game.publicData.stones;
			var _stone = null;
			for (var i in stones) {
				if (stones[i].maxLevel > item.data.level){
					if (player.storage.stone[i] > 0) 
						_stone = stones[i];
					break;
				}
			}

			if (tool.isUndefinedOrNull(_stone))
				return callback(new this.game.GameWarning('E0033',item.nam));


			player.data.gold -= _useGold;
			player.storage.stone[_stone.id] -=1;
			
			var _data = {};

			if (tool.checkRandom(_stone.percent, 100)) {
				item.data.level += 1;
				callback(null);

			} else {
				callback(new this.game.GameWarning('E0040', item.nam));
			}
			
			_data.chara={stone:player.storage.stone};
			_data.item={update:[item.data]};
			player.emit(_data);
			player.reflushchangeData();	
		...
		try {
			...
			switch (item.data.storage) {
				...
				case this.game.settings.game.storageType.equipment:
					...
					break;
				default:
					return callback(new this.game.GameWarning('E0031'));
			}

...
Team.prototype.leave = function (chara,callback){
	if (this.members[chara.uid]){
		...
		//退出socket.io room
		chara.socket.leave(chara.tmp.team.id);
		//移掉成員
		...
		//如果隊伍沒人
		if (this.count()<1) {
			console.log('leave out...............1');
			//移掉地圖上的隊伍
			Game.in('map_'+this.map.id,{team:{removeTeam:this.id}});
			delete this.map.teams[this.id];
		} else {
			console.log('leave out...............2');
			Game.in('map_'+this.map.id,{team:{updateTeam:Game.coms.team.parseReturnTeam(team)}});
		}
